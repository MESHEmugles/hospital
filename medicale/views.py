...
def newappoint(request):
    ...
    if request.method == 'POST':
        ...
        if apptform.is_valid():
            ...
            newapoint.save()
            # history= Appointmenthistory(appthistory_by= )
            # history.services = appty.services
            # history.booking_date= appty.apptdate
            # history.appt_date =appty.appt_time
            # history.follow_up = appty.follow_up
            # history.status = appty.status
            # history.amount = appty.price
            # history.save()
...
def delete_notice(request, slug, notice_id):
    ...
    return redirect ('loggin', context)
...
def paidorder(request):
    if request.method == 'POST':
        ...
        try:
            ...
            email = EmailMessage(
                'New Transaction alert !', #subject
                f'Admin, a client {first_names} {last_names}, with the phone number {phones} has just completed a transaction. \n kindly follow up on his transaction.', #message
                settings.EMAIL_HOST_USER, #HOST EMAIL
                ['mdpeter28@gmail.com'] #sender
            )
            email.fail_silently = True
